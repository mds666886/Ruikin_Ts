...
from app.models.entities import EvaluationRun, QaItem, Report, SceneSuggestion
...
SCENE_WEIGHTS = {
    "competition": {
        "focus": "创新点、技术路线、效果对比",
        "order_hint": "问题定义→方案亮点→实验结果→落地价值",
    },
    "thesis": {
        "focus": "研究问题、方法严谨性、实验设计",
        "order_hint": "研究背景→方法→实验→结论与不足",
    },
    "course": {
        "focus": "知识点完整性、逻辑清晰、时间控制",
        "order_hint": "目录→核心知识点→案例→总结",
    },
}


def generate_scene_suggestions(project_id: int, scene_type: str):
    SceneSuggestion.query.filter_by(project_id=project_id).delete()

    slides = db.session.execute(
        db.text(
            "SELECT s.page_no, s.title_guess, s.text_count, s.risk_tags FROM slides s "
            "JOIN files f ON s.file_id=f.id WHERE f.project_id=:pid ORDER BY s.page_no"
        ),
        {"pid": project_id},
    ).mappings().all()

    conf = SCENE_WEIGHTS.get(scene_type, SCENE_WEIGHTS["competition"])
    suggestions = [
        ("highlight", f"当前场景重点：{conf['focus']}", 1),
        ("order", f"建议讲述顺序：{conf['order_hint']}", 1),
    ]

    dense_pages = [s for s in slides if s["text_count"] > 260]
    if dense_pages:
        page_list = "、".join([f"第{s['page_no']}页" for s in dense_pages[:5]])
        suggestions.append(("cut", f"建议先精简 {page_list}，每页控制 6-8 行", 1))

    for s in slides[:3]:
        suggestions.append(("highlight", f"第{s['page_no']}页《{s['title_guess']}》建议补充一句过渡说明", 2))

    for kind, content, priority in suggestions:
        db.session.add(
            SceneSuggestion(
